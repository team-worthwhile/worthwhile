...
class InterpreterASTNodeVisitor extends HierarchialASTNodeVisitor {
	...
	public void visit(final ArrayLength arrayLength) {
		...
		final VariableDeclaration variable = ((VariableReference) arrayLength
				.getOperand()).getVariable();
	...
	public void visit(Assignment assignment) {
		...
		try {
			...
			this.setSymbol(assignment.getVariable().getVariable(),
					resultStack.pop());
		} catch (StatementException e) {
	...
	public void visit(FunctionCall functionCall) {
		...
		for (int i = 0; i < actuals.size(); i++) {
			...
			functionVisitor.setSymbol(functionDeclaration.getParameters()
					.get(i), this.resultStack.pop());
		}
		...
		try {
			if (variableDeclaration.getInitialValue() != null) {
				...
				if (variableDeclaration.getType() instanceof ArrayType) {
					...
					final IntegerLiteral sizeLiteral = AstFactory.init()
							.createIntegerLiteral();
					sizeLiteral
							.setValue(((IntegerValue) this.resultStack.pop())
									.getValue());
					...
					if (arrayType.getBaseType() instanceof BooleanType) {
						this.setSymbol(variableDeclaration,
								new CompositeValue<BooleanValue>(
										new BooleanValue[this.popIntegerValue()
												.getValue().intValue()]));
					} else {
						this.setSymbol(variableDeclaration,
								new CompositeValue<IntegerValue>(
										new IntegerValue[this.popIntegerValue()
												.getValue().intValue()]));
					}
				} else if (variableDeclaration.getType() instanceof BooleanType) {
					this.setSymbol(variableDeclaration, new BooleanValue(
							Boolean.FALSE));
				} else {
					this.setSymbol(variableDeclaration, new IntegerValue(
							BigInteger.ZERO));
				}
			}
		} catch (StatementException e) {
