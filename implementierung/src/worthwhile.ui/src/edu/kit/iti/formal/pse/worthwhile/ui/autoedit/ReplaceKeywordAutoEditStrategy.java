...
/**
 * The "Replace Keyword" auto edit strategy is meant to replace a keyword
 * typed in the editor with another string.
 * 
 * @author Joachim
 */
class ReplaceKeywordAutoEditStrategy implements IAutoEditStrategy {
    ...
    /**
     * Creates a new instance of the {@link ReplaceKeywordAutoEditStrategy}
     * class.
     * 
     * @param keyword
     *            The keyword that shall be replaced upon entry.
    ...
    public ReplaceKeywordAutoEditStrategy(String keyword, String replacement) {
    	...
    	throw new IllegalArgumentException(
    		"Keyword must be longer than one character.");
        }
    ...
    public void customizeDocumentCommand(final IDocument document,
    	...
    	try {
    		...
    		if (beginOfKeyword <= 0
    		    ...
    		    document.replace(command.offset
    			    - (keyword.length() - 1),
    			    (keyword.length() - 1), "");
